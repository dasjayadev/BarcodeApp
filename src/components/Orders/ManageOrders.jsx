        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {orders.map(order => {
            ...
            return (
              <div 
                key={order._id} 
                className={`border rounded-lg shadow-md overflow-hidden ${getStatusColor(order.status)} ${locked ? 'opacity-80' : ''}`}
              >
                <div className="p-4">
                  ...
                  {order.status === 'completed' && order.paymentStatus === 'paid' && (
                    <div className={`text-xs mb-2 font-medium ${locked ? 'text-red-600' : 'text-blue-600'}`}>
                      {locked ? (
